...
export default function ResetPasswordClient({ token }: { token: string }) {
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        if (!token) {
            toast.error(t("invalid_link_desc"));
            return;
        }
        ...
        <div className="min-h-screen flex items-center justify-center bg-white relative overflow-hidden p-4">
            ...
            >
                ...
                <div className="absolute -top-12 -right-8 z-20 hidden md:block">
                    <PopArtSticker text="NEW PASSWORD" color="bg-accent" className="text-sm rotate-12 scale-110" />
                </div>
                <div className="absolute -bottom-6 -left-12 z-20 hidden md:block">
                    <PopArtSticker text="SAFE" color="bg-info" className="text-xs -rotate-12" />
                </div>
                ...
                <div className="mt-8">
                    <Card className="border-4 border-black shadow-[8px_8px_0px_0px_#000] md:shadow-[16px_16px_0px_0px_#000] bg-white overflow-hidden rounded-[2rem] md:rounded-[2.5rem]">
                        <CardHeader className="space-y-2 bg-secondary/10 border-b-4 border-black p-5 md:p-8">
                            <CardTitle className="text-3xl md:text-4xl font-black uppercase tracking-tight text-foreground">{t("new_password")}</CardTitle>
                            ...
                            {!token ? (
                                ...
                                <form onSubmit={handleSubmit} className="space-y-6">
                                    <div className="space-y-3">
                                        <Label htmlFor="password" className="font-black uppercase text-[10px] md:text-xs tracking-widest ml-1 text-foreground/70">{t("new_password")}</Label>
                                    ...
                                    <div className="space-y-3">
                                        <Label htmlFor="confirmPassword" className="font-black uppercase text-[10px] md:text-xs tracking-widest ml-1 text-foreground/70">{t("confirm_password")}</Label>
                                        <div className="relative">
                                            <div className="absolute left-4 top-1/2 -translate-y-1/2 z-10">
                                    ...
                                    <div className="flex justify-center pt-2">
                                        <Link href="/login-admin" className="flex items-center gap-2 text-foreground font-black uppercase text-[10px] md:text-xs tracking-widest hover:text-primary transition-colors">
                                            <ArrowLeft className="h-4 w-4" /> {t("back_to_hq")}
                                        </Link>
                                    </div>
                                </form>
                            )}
