                ...
                <div className="flex items-center justify-between p-6 border-b border-border/20">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-0.5">
                            Sube las fotos de antes y después del grooming.
                        </p>
                    </div>
                    <button 
                        onClick={onClose} 
                        title="Cerrar"
                        aria-label="Cerrar"
                        className="h-9 w-9 flex items-center justify-center rounded-full bg-muted hover:bg-muted/80"
                    >
                        <X className="h-4 w-4" />
                    </button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                            <Label className="font-bold">🇪🇸 Título en Español</Label>
                            <Input
                                value={titleEs}
                        ...
                        <div className="space-y-2">
                            <Label className="font-bold">🇺🇸 Title in English</Label>
                            <Input
                                value={titleEn}
                        ...
                        <div className="space-y-2 flex flex-col justify-end pb-1">
                            ...
                            <div className="flex items-center gap-3">
                                <Switch
                                    ...
                                    id="visible-switch"
                                />
                                <label htmlFor="visible-switch" className="text-sm font-semibold cursor-pointer">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {/* Before */}
                        <div className="space-y-2">
                            <Label className="font-bold">📸 Foto ANTES</Label>
                            <label className="block cursor-pointer">
                                <div className="relative aspect-square rounded-2xl overflow-hidden border-2 border-dashed border-border/50 hover:border-primary/50 transition-colors bg-muted/20">
                                    {beforePreview ? (
                                        <Image src={beforePreview} alt="Antes" fill unoptimized className="object-cover" />
                                    ) : (
                                        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-muted-foreground">
                                            <Upload className="h-8 w-8" />
                                            <span className="text-sm font-semibold">Subir imagen ANTES</span>
                                        </div>
                                    )}
                                    {beforePreview && (
                        ...
                        {/* After */}
                        <div className="space-y-2">
                            <Label className="font-bold">✨ Foto DESPUÉS</Label>
                            <label className="block cursor-pointer">
                                <div className="relative aspect-square rounded-2xl overflow-hidden border-2 border-dashed border-primary/30 hover:border-primary/60 transition-colors bg-primary/5">
                                    {afterPreview ? (
                                        <Image src={afterPreview} alt="Después" fill unoptimized className="object-cover" />
                                    ) : (
                                        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-muted-foreground">
                                            <ImageIcon className="h-8 w-8" />
                                            <span className="text-sm font-semibold">Subir imagen DESPUÉS</span>
                                        </div>
                                    )}
                                    {afterPreview && (
                                        <div className="absolute bottom-2 right-2 bg-primary/80 text-white text-xs font-black px-2 py-1 rounded-full">
                                            DESPUÉS
                                        </div>
                                    )}
                                </div>
