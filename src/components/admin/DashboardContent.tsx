...
export default function DashboardContent({ stats }: DashboardContentProps) {
    ...
    return (
        <motion.div
            ...
            initial="hidden"
            animate="show"
            className="space-y-6 pb-12 bg-transparent text-white"
        >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Telemetry Charts - Apple/Linear Style */}
                <motion.div variants={item} className="lg:col-span-2 space-y-6">
                    <Card className="border-[#3A3A3A] shadow-xl bg-[#1A1A1A] rounded-2xl overflow-hidden">
                        <CardHeader className="pb-4 border-b border-[#3A3A3A]/50 flex flex-row items-center justify-between">
                            <div className="flex items-center space-x-2">
                                <Activity className="h-4.5 w-4.5 text-[#00DDEB]" />
                                <h3 className="font-black text-xs uppercase tracking-wider text-white">Telemetría del Negocio</h3>
                            </div>
                            <span className="flex items-center gap-1.5 text-[9px] text-[#00DDEB] font-black bg-[#00DDEB]/10 border border-[#00DDEB]/25 px-2.5 py-1 rounded-xl uppercase tracking-wider">
                                <span className="h-1.5 w-1.5 rounded-full bg-[#00DDEB] animate-pulse" />
                                EN TIEMPO REAL
                            </span>
                        </CardHeader>
                        <CardContent className="p-6 space-y-8">
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="text-xs font-black text-[#00DDEB]">${maxIncomeVal.toLocaleString()} Max</span>
                                </div>
                                <div className="h-36 flex items-end justify-between gap-4 pt-4 border-b border-[#3A3A3A]/50 px-2">
                                    {monthlyIncome.map((bar, i) => (
                                        <div key={bar.month} className="flex-1 flex flex-col items-center gap-2 group h-full justify-end">
                                            <div className="w-full bg-[#252525] rounded-t-xl h-full flex items-end relative overflow-hidden">
                                                <div 
                                                    className={cn(
                                                        "w-full bg-[#00DDEB] rounded-t-xl transition-all duration-500 group-hover:opacity-90 relative",
                                                        `dynamic-bar-h-${i}`
                                                    )}
                                                >
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between items-center">
                                    <div className="flex items-center space-x-2">
                                        <CalendarCheck className="h-4 w-4 text-[#00DDEB]" />
                                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Fluctuación de Citas Semanal</span>
                                    </div>
                                    <span className="text-xs font-black text-[#00DDEB]">{maxTrendVal} Citas Max</span>
                                </div>
                                <div className="h-28 flex items-end justify-between gap-4 pt-4 border-b border-[#3A3A3A]/50 px-2">
                                    {appointmentTrend.map((point, i) => (
                                        <div key={point.day} className="flex-1 flex flex-col items-center gap-2 group h-full justify-end">
                                            <div className="w-full h-full flex flex-col justify-end items-center relative">
                                                ...
                                                <div 
                                                    className={cn(
                                                        "absolute h-3.5 w-3.5 rounded-full bg-[#00DDEB] border-2 border-[#1A1A1A] z-10 transition-all duration-300 group-hover:scale-125 shadow-[0_0_8px_rgba(0,221,235,0.4)]",
                                                        `dynamic-point-b-${i}`
                                                    )}
                                                >
                                                    {/* Pulse Ring for high performance emphasis */}
                                                    <span className="absolute -inset-1 rounded-full bg-[#00DDEB]/30 animate-ping pointer-events-none" />
                                                </div>
                                                {/* Tooltip */}
                    ...
                    {/* Banner Section - Apple Dark Theme */}
                    <div className="bg-[#1A1A1A] border border-[#3A3A3A] rounded-2xl p-6 md:p-8 flex flex-col justify-center text-white relative overflow-hidden shadow-xl">
                        <div className="absolute top-0 right-0 w-64 h-64 bg-[#00DDEB]/10 blur-[80px] -mr-32 -mt-32" />
                        <h3 className="text-xl md:text-2xl font-black tracking-tight mb-2 relative z-10 text-white">Gestión de la Plataforma</h3>
                        <p className="text-slate-400 text-xs md:text-sm max-w-lg font-bold relative z-10 leading-relaxed">
                        ...
                        <div className="mt-6 relative z-10 flex flex-col sm:flex-row flex-wrap gap-3">
                            <Link href="/admin/config" className="w-full sm:w-auto">
                                <Button className="w-full sm:w-auto rounded-xl px-5 h-11 font-black bg-[#252525] text-white border border-[#3A3A3A] hover:bg-[#2D2D2D] hover:text-white cursor-pointer text-xs uppercase tracking-wider">
                                    <Settings className="mr-2 h-4 w-4" /> Configuración Global
                                </Button>
                            </Link>
                            <Link href="/admin/finanzas" className="w-full sm:w-auto">
                                <Button className="w-full sm:w-auto rounded-xl px-5 h-11 font-black bg-[#00DDEB] text-black hover:bg-[#00DDEB]/90 cursor-pointer text-xs uppercase tracking-wider">
                ...
                <motion.div variants={item} className="space-y-6">
                    ...
                    <Card className="border-[#3A3A3A] shadow-xl bg-[#1A1A1A] rounded-2xl overflow-hidden">
                        <CardHeader className="pb-3 border-b border-[#3A3A3A]/50 flex flex-row items-center justify-between">
                            ...
                            <Link href="/admin/finanzas" className="text-[10px] font-black text-[#00DDEB] hover:underline uppercase tracking-wider">
                                Gestionar
                            </Link>
                        </CardHeader>
                    ...
                    <Card className="border-[#3A3A3A] shadow-xl bg-[#1A1A1A] rounded-2xl overflow-hidden">
                        <CardHeader className="pb-3 border-b border-[#3A3A3A]/50">
                            <div className="flex items-center space-x-2 text-white">
                                <LayoutDashboard className="h-4 w-4 text-[#00DDEB]" />
...
function StatCard({ icon: Icon, title, value, label, color, variants, href }: StatCardProps) {
    const content = (
        <Card className="group border-[#3A3A3A] shadow-xl hover:border-[#00DDEB]/40 transition-all duration-300 bg-[#1A1A1A] rounded-2xl overflow-hidden h-full">
            <CardHeader className="flex flex-row items-center justify-between pb-2 p-6">
                ...
                <div className={cn(
                    ...
                    color === "primary" 
                        ? "bg-[#00DDEB]/10 text-[#00DDEB] border-[#00DDEB]/25" 
