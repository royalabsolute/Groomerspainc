...
export default function QuoteSection({ locale, initialServices }: QuoteSectionProps) {
    ...
    const renderSummaryCard = (isMobileLayout: boolean = false) => {
        ...
        return (
            <div className={cn(
                "text-neutral-900 space-y-5 relative overflow-hidden transition-all",
                isMobileLayout 
                    ? "bg-white border-3 border-black p-5 rounded-2xl shadow-[4px_4px_0_0_#000]"
                    : "h-full flex flex-col justify-between"
            )}>
                <div className="absolute top-0 right-0 p-4 opacity-[0.03] text-black pointer-events-none">
                ...
                {/* Total Pricing Output with elastic pulse animation */}
                <div className="flex justify-between items-end border-t-3 border-black border-dashed pt-4 mt-2">
                    ...
                    <motion.span 
                        ...
                        className="text-3xl font-black text-neutral-900 leading-none inline-block"
                    >
    ...
    return (
        <section id="cotizar" className="py-20 bg-[#FDFCF8] relative overflow-hidden px-4 sm:px-6 lg:px-8 border-b-4 border-black">
            <div className="max-w-7xl mx-auto space-y-12">
                                ...
                                <AnimatePresence mode="wait">
                                    ...
                                    {step === 2 && (
                                        ...
                                        >
                                                            ...
                                                            >
                                                                ...
                                                                <div className="flex items-center gap-3">
                                                                    ...
                                                                    {isExpanded ? (
                                                                        ...
                                                                        <ChevronDown className="h-5 w-5 text-neutral-505" />
                                                                    )}
                                                                </div>
                                                            </div>
                                            ...
                                            >
                                                <PlusCircle className="h-4.5 w-4.5 animate-pulse" /> {locale === "es" ? "Añadir otra mascota" : "Add another pet"}
                                            </Button>
                                                    ...
                                                    return (
                                                        <div key={field.id} className="border-4 border-black p-5 rounded-2xl bg-white space-y-4 shadow-[4px_4px_0_0_#000]">
                                                            ...
                                                            <div className="space-y-2">
                                                                <Label className="font-black text-xs uppercase tracking-wider text-slate-750 flex items-center gap-1.5">
                ...
                <div className="fixed bottom-4 left-4 right-4 z-50 bg-white border-4 border-black rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-4 flex justify-between items-center md:hidden select-none">
                    <div className="flex items-center gap-2">
                        <div className="w-10 h-10 bg-amber-400 border-2 border-black rounded-xl flex items-center justify-center text-neutral-900 shadow-[2px_2px_0_0_#000] shrink-0">
                        ...
                        <div className="flex flex-col">
                            ...
                            <span className="font-black text-xs text-neutral-800 uppercase tracking-tight leading-none">
                                {petsList.length} {petsList.length === 1 ? (locale === "es" ? "Mascota" : "Pet") : (locale === "es" ? "Mascotas" : "Pets")}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        <div className="flex flex-col text-right">
                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-450 leading-none mb-0.5">
                                Total:
                            </span>
                            <motion.span 
                                key={discountedPrice}
                                ...
                                transition={{ duration: 0.3, ease: "easeOut" }}
                                className="font-black text-lg text-neutral-900 leading-none inline-block"
                            >
                                ${discountedPrice.toFixed(2)}
                            </motion.span>
                        </div>
                        ...
                        {step < 3 ? (
                            <Button 
                                ...
                                className="bg-[#06B6D4] text-white font-black h-10 px-4 rounded-xl border-2 border-black shadow-[2px_2px_0_0_#000] text-xs uppercase flex items-center justify-center gap-1 cursor-pointer transition-all active:translate-y-px"
                            >
                                <span>{locale === "es" ? "Sig" : "Next"}</span> 
                                <ChevronRight className="h-4 w-4 shrink-0 inline" />
                            </Button>
                        ) : (
                            <Button 
                                type="button"
                                ...
                                className="bg-[#2ECC71] text-neutral-900 font-black h-10 px-4 rounded-xl border-2 border-black shadow-[2px_2px_0_0_#000] text-xs uppercase flex items-center justify-center gap-1 cursor-pointer transition-all active:translate-y-px"
                            >
                                <span>{locale === "es" ? "Fin" : "Confirm"}</span>
                                <Check className="h-4 w-4 shrink-0 inline" />
                            </Button>
                        )}
                    </div>
                </div>
